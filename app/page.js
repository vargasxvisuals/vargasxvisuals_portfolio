    ...
    <main style={{background:'#f7f3ee', minHeight:'100vh', color:'#2c2420'}}>
      ...
      <nav style={{position:'fixed', top:0, left:0, right:0, zIndex:100, padding:'20px 48px', display:'flex', justifyContent:'space-between', alignItems:'center', background:'rgba(247,243,238,0.95)', backdropFilter:'blur(8px)', borderBottom:'1px solid #e8e0d8'}}>
        ...
        <div style={{display:'flex', gap:'36px'}}>
          {[['Portfolio','/portfolio'],['About','/about'],['Contact','/contact']].map(([l,h]) => (
      ...
      <section style={{display:'grid', gridTemplateColumns:'55% 45%', minHeight:'100vh', paddingTop:'0'}}>
        <div style={{position:'relative', overflow:'hidden'}}>
          <img
            src={photos.hero}
            alt="Couple in golden California wildflower field"
            style={{width:'100%', height:'100%', objectFit:'cover', objectPosition:'center'}}
          />
          <div style={{position:'absolute', inset:0, background:'linear-gradient(to right, transparent 70%, rgba(247,243,238,0.3))'}} />
        </div>
        <div style={{display:'flex', flexDirection:'column', justifyContent:'center', padding:'80px 56px 80px 48px', background:'#f7f3ee'}}>
          ...
          <div style={{display:'flex', gap:'16px', flexWrap:'wrap'}}>
            <Link href="/contact" style={{padding:'14px 40px', background:'#2c2420', color:'#f7f3ee', textDecoration:'none', fontSize:'11px', letterSpacing:'3px', textTransform:'uppercase'}}>
              Book a Session
            </Link>
      ...
      <section style={{padding:'80px 48px'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1.3fr 1fr 1fr', gridTemplateRows:'280px 280px', gap:'6px', maxWidth:'1200px', margin:'0 auto'}}>
          <div style={{gridRow:'span 2', overflow:'hidden'}}>
            <img src={photos.field} alt="Couple in wildflower field" style={{width:'100%', height:'100%', objectFit:'cover', display:'block', transition:'transform 0.6s ease'}}
              onMouseEnter={e=>e.target.style.transform='scale(1.03)'}
              onMouseLeave={e=>e.target.style.transform='scale(1)'}
            />
          </div>
          <div style={{overflow:'hidden'}}>
            <img src={photos.wedding} alt="Wedding couple editorial portrait" style={{width:'100%', height:'100%', objectFit:'cover', objectPosition:'top', display:'block', transition:'transform 0.6s ease'}}
              onMouseEnter={e=>e.target.style.transform='scale(1.03)'}
              onMouseLeave={e=>e.target.style.transform='scale(1)'}
            />
          </div>
          <div style={{overflow:'hidden'}}>
            <img src={photos.intimate} alt="Intimate couple portrait" style={{width:'100%', height:'100%', objectFit:'cover', objectPosition:'top', display:'block', transition:'transform 0.6s ease'}}
              onMouseEnter={e=>e.target.style.transform='scale(1.03)'}
              onMouseLeave={e=>e.target.style.transform='scale(1)'}
            />
          </div>
          <div style={{overflow:'hidden', gridColumn:'span 2'}}>
            <img src={photos.sunset} alt="Couple at golden hour" style={{width:'100%', height:'100%', objectFit:'cover', objectPosition:'center 30%', display:'block', transition:'transform 0.6s ease'}}
              onMouseEnter={e=>e.target.style.transform='scale(1.03)'}
              onMouseLeave={e=>e.target.style.transform='scale(1)'}
            />
          </div>
      ...
      <section style={{padding:'100px 48px', background:'#f7f3ee'}}>
        ...
        <h2 style={{fontFamily:'Georgia, serif', fontSize:'clamp(28px, 3vw, 44px)', fontWeight:'400', textAlign:'center', marginBottom:'56px', color:'#2c2420'}}>Every celebration, beautifully told</h2>
        <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:'3px', maxWidth:'1200px', margin:'0 auto'}}>
          {[
      ...
      <section style={{padding:'120px 48px', textAlign:'center', background:'#f7f3ee'}}>
        ...
        <p style={{color:'#9a8070', marginBottom:'48px', fontSize:'15px', letterSpacing:'1px'}}>Fresno, CA — available throughout all of California</p>
        <Link href="/contact" style={{padding:'18px 56px', background:'#b8864a', color:'#f7f3ee', textDecoration:'none', fontSize:'11px', letterSpacing:'3px', textTransform:'uppercase'}}>
          Get in Touch
        </Link>
      </section>
